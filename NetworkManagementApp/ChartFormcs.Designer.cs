namespace NetworkManagementApp
{
    partial class ChartFormcs
    {
        /// <summary>
        /// Required designer variable.
        /// </summary>
        private System.ComponentModel.IContainer components = null;

        /// <summary>
        /// Clean up any resources being used.
        /// </summary>
        /// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
        protected override void Dispose(bool disposing)
        {
            if (disposing && (components != null))
            {
                components.Dispose();
            }
            base.Dispose(disposing);
        }

        #region Windows Form Designer generated code

        /// <summary>
        /// Required method for Designer support - do not modify
        /// the contents of this method with the code editor.
        /// </summary>
        private void InitializeComponent()
        {
            panelHistograma = new Panel();
            SuspendLayout();
            // 
            // panelHistograma
            // 
            panelHistograma.Location = new Point(28, 18);
            panelHistograma.Name = "panelHistograma";
            panelHistograma.Size = new Size(469, 289);
            panelHistograma.TabIndex = 0;
            panelHistograma.Paint += ChartFormcs_Paint;
            // 
            // ChartFormcs
            // 
            AutoScaleMode = AutoScaleMode.None;
            ClientSize = new Size(528, 373);
            Controls.Add(panelHistograma);
            Name = "ChartFormcs";
            Text = "ChartFormcs";
            //Paint += ChartFormcs_Paint;
            ResumeLayout(false);
        }

        #endregion

        private Panel panelHistograma;
    }
}